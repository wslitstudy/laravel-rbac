...
class IniMeau extends Command
{

    const WHITE_ARR = ['IndexController', 'LoginController'];

    protected $signature = 'ini_admin_menu';
            ...
            if ($method->isPublic() && $method->class == 'App\Http\Controllers\Admin\\' . $className) {
                ...
                if ($doc) {
                    ...
                    if ($meauid && !ManagePower::checkPath($path)) {
                        \App\Lib\Model\Rbac\Power\Service::addPower($path, $result['method'], $meauid);
                    }
                }
            }
        }

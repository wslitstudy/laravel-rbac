...
var main = {
    waringAlert: function (message) {
        var content = '<div class="text-center" style="border-top:1px solid #eee;padding-top: 20px">' + message + '</div>';
        $.alert({
    ...
    ajaxPosts: function (event) {
        var url = $(event).attr('data-url');
        var data = $(event).parents('form').serialize();
        var obj = $(event);
        obj.attr('disabled', true);
        main.ajaxPost(data, url);
        obj.attr('disabled', false);
    },
    ajaxPost: function (data, url) {
        var urls = url ? url : '';
        $.ajax({
            url: urls,
            method: 'POST',
            ...
            error: function (response) {
                if (response.status != 200) {
                    ...
                    main.waringAlert(responseText.message);
                }
            }
        })
    },
